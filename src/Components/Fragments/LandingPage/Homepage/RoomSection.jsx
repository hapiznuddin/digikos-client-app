...
const RoomSection = () => {
  ...
  return data?.data.map((room) => {
    ...
    return (
      <div
        key={room.id}
        className="card w-1/4 rounded-3xl bg-base-100 shadow-xl"
        data-aos="zoom-in-up"
      ...
      >
        {isLoading ? (
          <>
            ...
            <div className="flex w-full py-5 px-8 gap-8">
              <SkeletonText className="flex flex-col gap-2 w-1/2 ">
                <h2 className="font-bold text-xl">{room.room_name}</h2>
                <p className="overflow-hidden max-h-20 text-xs line-clamp-5">
                  {room.room_description}
                </p>
              </SkeletonText>
              <div className="card-actions w-1/2 flex flex-col justify-between items-end ">
                <SkeletonText className="flex gap-1">
                  <p className="text-neutral-800 text-lg font-semibold">4.5</p>
                ...
                <Skeleton>
                  <ButtonPrimary className="w-2/3">Lihat Kamar</ButtonPrimary>
                </Skeleton>
              </div>
            </div>
          </>
